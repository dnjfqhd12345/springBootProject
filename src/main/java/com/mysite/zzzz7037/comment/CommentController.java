...
import com.mysite.zzzz7037.question.QuestionService;
...
public class CommentController {
    ...
    public String createQuestionComment(@PathVariable("id") Integer id,
        ...
        Optional<Question> question = questionService.getQuestionOptional(id);
        Optional<SiteUser> user = userService.getUserOptional(principal.getName());
        if(question.isPresent() && user.isPresent()) {
            if(bindingResult.hasErrors()){
                return "comment_form";
            }
            Comment c = commentService.create(question.get(), user.get(),
